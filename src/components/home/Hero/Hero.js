...
function Hero() {
  ...
  function heroAccessClick() {
    let heroEmailInput = document.querySelector(".hero-email-input input");
    if (
      /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(
        heroEmail
      )
  ...
  let [heroEmail, setHeroEmail] = React.useState("");

  function heroFormInputChange(event) {
    setHeroEmail(event.target.value);
  }
  let [emailTruthy, setEmailTruthy] = React.useState(false);
  let heroFormAccess = (
    <div className="hero-email-input-container">
      <div className="hero-email-input">
        <input
          onChange={heroFormInputChange}
          value={heroEmail}
          type="email"
          placeholder="Email"
          required
  ...
  let heroFormSuccess = (
    <div className="hero-email-input-container">
      <p>Thank you! Your submission has been recieved!</p>
    </div>
  );
  ...
  return (
    <div className="hero" id="Hero">
      ...
      <h1 className="hero-heading">Instant collaboration for remote teams</h1>
      <p className="hero-paragraph">
        All-in-one place for your remote team to chat, collaborate and track
        project progress.
      </p>
      {!emailTruthy && heroFormAccess}
      {emailTruthy && heroFormSuccess}
    </div>
  );
}
